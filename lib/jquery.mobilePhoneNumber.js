...
(function() {
  var $, checkForCountryChange_, formatBack_, formatForPhone_, formatUp_, format_, formats, formattedPhoneNumber_, formattedPhone_, getNewCaretPosition_, isEventAllowedChar_, isEventAllowed_, mobilePhoneNumber, prefixesAreSubsets_, restrictEventAndFormat_, supportSelectionEnd,
    __slice = [].slice;
  ...
  format_ = function(value, e) {
    var phone, selection, selectionAtEnd, textBeforeCaret;
    phone = formattedPhone_.call(this, value, true);
    if (phone !== this.val()) {
      textBeforeCaret = value.slice(0, this.caret() + 1).replace(/\D+/g, '');
      e.preventDefault();
      this.val(phone);
  ...
  getNewCaretPosition_ = function(textBeforeCaret) {
    var caretPosition, char, _i, _len, _ref;
    if (!textBeforeCaret) {
